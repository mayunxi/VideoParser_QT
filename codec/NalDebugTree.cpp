...
// 树形项
TREE_NALU NalParser::AddTreeItem(TREE_NALU _item)
{
    QTreeWidgetItem *item = new QTreeWidgetItem(_item);    //创建第一个父节点
    item->setText(0,m_outputInfo);
    //item->setCheckState(0,Qt::Unchecked);
    item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsUserCheckable|Qt::ItemIsEnabled);
    return item;
    //m_pTree->InsertItem(m_outputInfo, _item);
}

TREE_NALU NalParser::AddTreeItem(QTreeWidget *tree)
{
    QTreeWidgetItem *item = new QTreeWidgetItem(tree);    //创建第一个父节点
    item->setText(0,m_outputInfo);
    //item->setCheckState(0,Qt::Unchecked);
    item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsUserCheckable|Qt::ItemIsEnabled);
    return item;
...
void NalParser::h264_debug_sps(sps_t* sps, TREE_NALU  root)
{
    my_printf("seq_parameter_set_data()");
    TREE_NALU  isps = AddTreeItem(root);
        ...
        sps->profile_idc == 122 || sps->profile_idc == 144 )
    {
        ...
        TREE_NALU  ssmpf = AddTreeItem(isps);
    ...
    TREE_NALU  fmof = AddTreeItem(isps);
    ...
    TREE_NALU  fcf = AddTreeItem(isps);
    ...
    if (sps->vui_parameters_present_flag)
    {
        ...
        TREE_NALU  vstpf = AddTreeItem(vp);
        if( sps->vui.video_signal_type_present_flag )
        {
            ...
            TREE_NALU  cdpf = AddTreeItem(vstpf);
        ...
        TREE_NALU  clipf = AddTreeItem(vp);
        ...
        TREE_NALU  nhppf = AddTreeItem(vp);
        if( sps->vui.nal_hrd_parameters_present_flag )
        {
            my_printf("hrd_parameters()");
            TREE_NALU  hp = AddTreeItem(nhppf);
        ...
        TREE_NALU  vhppf = AddTreeItem(vp);
        if( sps->vui.vcl_hrd_parameters_present_flag )
        {
            my_printf("hrd_parameters()");
            TREE_NALU  hp = AddTreeItem(vhppf);
            my_printf("cpb_cnt_minus1: %d  (v bits)", sps->hrd.cpb_cnt_minus1 );AddTreeItem(hp);
            my_printf("bit_rate_scale: %d  (4 bits)", sps->hrd.bit_rate_scale );AddTreeItem(hp);
            my_printf("cpb_size_scale: %d  (4 bits)", sps->hrd.cpb_size_scale );AddTreeItem(hp);
            int SchedSelIdx;
...
void NalParser::h264_debug_pps(pps_t* pps, TREE_NALU  root)
{
    my_printf("pic_parameter_set_rbsp()");
    TREE_NALU  ipps = AddTreeItem(root);
    ...
    if( pps->_more_rbsp_data_present )
    {
        ...
        TREE_NALU  imrdp = AddTreeItem(ipps);
        ...
        TREE_NALU  psmpf = AddTreeItem(imrdp);
...
void NalParser::h264_debug_slice_header(h264_stream_t* h, TREE_NALU  root)
{
    sps_t* sps = h->sps;
    pps_t* pps = h->pps;
    ...
    TREE_NALU  slice = AddTreeItem(root);
    my_printf("slice_header()");
    TREE_NALU  iheader = AddTreeItem(slice);
    ...
    TREE_NALU  ppsid = AddTreeItem(iheader);
    ...
    if( !sps->frame_mbs_only_flag )
    {
        ...
        TREE_NALU  fpf = AddTreeItem(iheader);
    ...
    if (nal->nal_unit_type == 20 || nal->nal_unit_type == 21)
    {
        ...
        TREE_NALU  rplmm = AddTreeItem(iheader);
    }
    else
    {
        my_printf("ref_pic_list_modification()");
        TREE_NALU  rplm = AddTreeItem(iheader);
        ...
        ( pps->weighted_bipred_idc == 1 && is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) ) )
    {
        ...
        TREE_NALU  ipwt = AddTreeItem(iheader);
        ...
        for( int i = 0; i <= sh->num_ref_idx_l0_active_minus1; i++ )
        {
            ...
            TREE_NALU  ilwl0f = AddTreeItem(ilwl0);
        ...
        for( int i = 0; i <= sh->num_ref_idx_l0_active_minus1; i++ )
        {
            if ( sps->ChromaArrayType != 0 )
            {
                ...
                TREE_NALU  icwl0f = AddTreeItem(icwl0);
        ...
        if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) )
        {
            ...
            for( int i = 0; i <= sh->num_ref_idx_l1_active_minus1; i++ )
            {
                ...
                TREE_NALU  ilwl1f = AddTreeItem(ilwl1);
            ...
            for( int i = 0; i <= sh->num_ref_idx_l1_active_minus1; i++ )
            {
                if ( sps->ChromaArrayType != 0 )
                {
                    ...
                    TREE_NALU  icwl1f = AddTreeItem(icwl1);
    ...
    if( nal->nal_ref_idc != 0 )
    {
        ...
        TREE_NALU  idrpm = AddTreeItem(iheader);
        ...
        else
        {
            ...
            TREE_NALU  arpmmf = AddTreeItem(idrpm);
...
void NalParser::h264_debug_aud(aud_t* aud, TREE_NALU  root)
{
    my_printf("access_unit_delimiter_rbsp()");
    TREE_NALU  iaud = AddTreeItem(root);
...
void NalParser::h264_debug_seis( h264_stream_t* h, TREE_NALU  root)
{
    sei_t** seis = h->seis;
    ...
    my_printf("sei_rbsp()");
    TREE_NALU  isei = AddTreeItem(root);
    my_printf("sei_message()");
    TREE_NALU  iisei = AddTreeItem(isei);
    ...
    for (i = 0; i < num_seis; i++)
    {
        ...
        TREE_NALU  sp = AddTreeItem(iisei);
 ...
 */
void NalParser::h264_debug_nal_t(h264_stream_t* h, nal_t* nal)
{
    if (m_root)
    {
        int itemChildrenCounts = m_root->childCount();
        while(itemChildrenCounts--)
        {
             QTreeWidgetItem * child = m_root->child(itemChildrenCounts); //index从大到小区做删除处理
    ...
    my_printf("NAL");
   TREE_NALU  root = AddTreeItem(m_pTree);
   m_root = root;
...
////////////////////////////////////////////////////////
void NalParser::h265_debug_ptl(profile_tier_level_t* ptl, int profilePresentFlag, int max_sub_layers_minus1, TREE_NALU  root)
{
...
void NalParser::h265_debug_sub_layer_hrd_parameters(sub_layer_hrd_parameters_t* subhrd, int sub_pic_hrd_params_present_flag, int CpbCnt, int subLayerId, const char* p, TREE_NALU  root)
{
    my_printf("[%s] sub_layer_hrd_parameters(%d)", p, subLayerId);
    TREE_NALU  slhp = AddTreeItem(root);
...
void NalParser::h265_debug_hrd_parameters(hrd_parameters_t* hrd, int commonInfPresentFlag, int maxNumSubLayersMinus1, TREE_NALU  root)
{
    my_printf("hrd_parameters()");
    TREE_NALU  ihrd = AddTreeItem(root);
...
// vps
void NalParser::h265_debug_vps(h265_vps_t* vps, TREE_NALU  root)
{
    int i, j;
    my_printf("video_parameter_set_rbsp()");
    TREE_NALU  ivps = AddTreeItem(root);
    ...
    TREE_NALU  iptl = AddTreeItem(ivps);
    ...
    {
        my_printf("SubLayers");
        TREE_NALU  isub = AddTreeItem(ivps);
    ...
    TREE_NALU  tipf = AddTreeItem(ivps);
    if (vps->vps_timing_info_present_flag)
    {
        ...
        TREE_NALU  ppttf = AddTreeItem(tipf);
...
void NalParser::h265_debug_scaling_list(scaling_list_data_t* sld, TREE_NALU  root)
{
    my_printf("scaling_list_data()");
    TREE_NALU  isl = AddTreeItem(root);
...
void NalParser::h265_debug_short_term_ref_pic_set(h265_sps_t* sps, st_ref_pic_set_t*st, referencePictureSets_t* rps, int stRpsIdx, TREE_NALU  root)
{
    my_printf("short_term_ref_pic_set( %d )", stRpsIdx);
    TREE_NALU  srps = AddTreeItem(root);
    TREE_NALU  srpsii = srps;
    if (stRpsIdx != 0)
    {
...
void NalParser::h265_debug_vui_parameters(vui_parameters_t* vui, int maxNumSubLayersMinus1, TREE_NALU  root)
{
    my_printf("vui_parameters()");
    TREE_NALU  ivp = AddTreeItem(root);
    my_printf_flag("aspect_ratio_info_present_flag", vui->aspect_ratio_info_present_flag);
    TREE_NALU  aripf = AddTreeItem(ivp);
    ...
    TREE_NALU  oipf = AddTreeItem(ivp);
    ...
    TREE_NALU  vstpf = AddTreeItem(ivp);
    ...
    TREE_NALU  clipf = AddTreeItem(ivp);
    ...
    TREE_NALU  ddwf = AddTreeItem(ivp);
    ...
    TREE_NALU  vtipf = AddTreeItem(ivp);
    if (vui->vui_timing_info_present_flag)
    {
        ...
        TREE_NALU  vppttf = AddTreeItem(vtipf);
        ...
        TREE_NALU  vhppf = AddTreeItem(vtipf);
    ...
    TREE_NALU  brf = AddTreeItem(vtipf);
...
// sps
void NalParser::h265_debug_sps(h265_sps_t* sps, TREE_NALU  root)
{
    ...
    // ptl
    my_printf("profile_tier_level()");
    TREE_NALU  iptl = AddTreeItem(isps);
    ...
    TREE_NALU  cwf = AddTreeItem(isps);
    ...
    my_printf("SubLayers");
    TREE_NALU  isloripf = AddTreeItem(isps);
    ...
    TREE_NALU  pef = AddTreeItem(isps);
    ...
    TREE_NALU  ltrppf = AddTreeItem(isps);
    ...
    if (sps->sps_extension_present_flag)
    {
        ...
        TREE_NALU  smef = AddTreeItem(sepf);
...
// pps
void NalParser::h265_debug_pps(h265_pps_t* pps, TREE_NALU  root)
{
    my_printf("pic_parameter_set_rbsp()");
    TREE_NALU  ipps = AddTreeItem(root);
    ...
    TREE_NALU  cqdef = AddTreeItem(ipps);
    ...
    TREE_NALU  tef = AddTreeItem(ipps);
    ...
    TREE_NALU  dfcpf = AddTreeItem(ipps);
    if (pps->deblocking_filter_control_present_flag)
    {
        ...
        TREE_NALU  dfdf = AddTreeItem(dfcpf);
    ...
    TREE_NALU  sldf = AddTreeItem(ipps);
...
// aud
void NalParser::h265_debug_aud(h265_aud_t* aud, TREE_NALU  root)
{
...
// sei
void NalParser::h265_debug_seis(h265_stream_t* h, TREE_NALU  root)
{
...
void NalParser::h265_debug_ref_pic_lists_modification(h265_slice_header_t* hrd, TREE_NALU  root)
{
...
void NalParser::h265_debug_pred_weight_table(h265_stream_t* h, TREE_NALU  root)
{
    pred_weight_table_t* pwt = &h->sh->pred_weight_table;
    h265_sps_t* sps = h->sps;

    my_printf("pred_weight_table()");
    TREE_NALU  ipwt = AddTreeItem(root);
...
void NalParser::h265_debug_slice_header(h265_stream_t* h, TREE_NALU  root)
{
    ...
    TREE_NALU  islce = AddTreeItem(root);

    my_printf("slice_segment_header()");
    TREE_NALU  ihrd = AddTreeItem(islce);
    ...
    if (!hrd->dependent_slice_segment_flag)
    {
        ...
        TREE_NALU  dssf = AddTreeItem(ihrd);
        ...
        if (hrd->slice_type == H265_SH_SLICE_TYPE_P || hrd->slice_type == H265_SH_SLICE_TYPE_B)
        {
            ...
            TREE_NALU  nriaof = AddTreeItem(dssf); // 根据H265VideoESViewer，没有节点
            ...
            {
                h265_debug_ref_pic_lists_modification(hrd,nriaof);
            }
            if (hrd->slice_type == H265_SH_SLICE_TYPE_B)
            {
        ...
        if (hrd->deblocking_filter_override_flag)
        {
            ...
            TREE_NALU  dfof = AddTreeItem(dssf);
    ...
    if (pps->tiles_enabled_flag || pps->entropy_coding_sync_enabled_flag)
    {
        ...
        TREE_NALU  inepo = AddTreeItem(ihrd);
        if (hrd->num_entry_point_offsets > 0)
        {
            ...
            TREE_NALU  iinepo = AddTreeItem(ihrd);
...
void NalParser::h265_debug_nal_t(h265_stream_t* h, h265_nal_t* nal)
{
    int my_nal_type = -1;
    ...
    TREE_NALU  subroot = AddTreeItem(root);
